...
class ProcessorDesignTool {
  ...
  setupEventListeners() {
    ...
    document.getElementById('load-example').addEventListener('click', () => this.showExampleModal());
    document.getElementById('load-design').addEventListener('click', () => this.loadDesign());
    document.getElementById('validate-design').addEventListener('click', () => this.validateDesign());
    document.getElementById('compile-design').addEventListener('click', () => this.showCompilationPanel());
    ...
    document.getElementById('close-compilation')?.addEventListener('click', () => {
    ...
    document.getElementById('start-compilation')?.addEventListener('click', () => this.compileDesign());
    ...
    document.getElementById('connection-mode').addEventListener('change', (e) => {
    ...
    document.getElementById('design-file-input').addEventListener('change', (e) => this.handleFileLoad(e));
  }
  ...
  getConnectionPoints(type) {
    ...
    const points = {
      ...
      'cache': [
        ...
        { id: 'mem_out', x: 100, y: 40, type: 'output', label: 'Memory Request' }
      ],
      'decoder': [
      ...
      'clock': [
        { id: 'clk_out', x: 100, y: 30, type: 'output', label: 'Clock' }
      ],
      'input-port': [
      ...
      'interrupt': [
        ...
        { id: 'cpu_int', x: 100, y: 30, type: 'output', label: 'CPU Interrupt' }
      ],
      'adder': [
        { id: 'in1', x: 0, y: 20, type: 'input', label: 'A' },
        { id: 'in2', x: 0, y: 40, type: 'input', label: 'B' },
      ...
      'shifter': [
        ...
        { id: 'data_out', x: 100, y: 30, type: 'output', label: 'Shifted Data' }
      ],
      'comparator': [
        { id: 'in1', x: 0, y: 20, type: 'input', label: 'A' },
        { id: 'in2', x: 0, y: 40, type: 'input', label: 'B' },
      ...
      'instruction-register': [
        { id: 'instruction_in', x: 0, y: 20, type: 'input', label: 'Instruction' },
      ...
      'stack-pointer': [
        ...
        { id: 'address', x: 100, y: 30, type: 'output', label: 'Stack Address' }
      ]
    };
    return points[type] || [];
  }
  ...
  canConnect(start, end) {
    return start.component !== end.component &&
  ...
  handleFileLoad(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        this.showNotification('Error loading design file. Please check the file format.', 'error');
      }
    };
  ...
  validateDesign() {
    ...
    if (hasALU && hasMemory && hasControl && hasRegister) {
      results.push({
        ...
        message: 'Design contains all essential processor components (ALU, Memory, Control Unit, Register).'
      });
    } else {
      results.push({
        type: 'warning',
    ...
    if (hasCycles) {
      results.push({
        ...
        message: 'Design contains circular connections which may cause timing issues.'
      });
    } else {
      results.push({
        type: 'success',
  ...
  dfsHasCycle(component, visited, recursionStack) {
    ...
    const outputConnections = this.connections.filter(conn => 
      conn.start.component === component
    );
...
; based on your processor design components.`;
  ...
  createSimple8BitCPU() {
    const components = [
      { id: 'alu1', type: 'alu', x: 300, y: 200, width: 100, height: 60, icon: '🧮', label: 'ALU', connectionPoints: this.getConnectionPoints('alu') },
      { id: 'reg1', type: 'register', x: 150, y: 150, width: 100, height: 60, icon: '📝', label: 'REGISTER', connectionPoints: this.getConnectionPoints('register') },
      { id: 'reg2', type: 'register', x: 150, y: 250, width: 100, height: 60, icon: '📝', label: 'REGISTER', connectionPoints: this.getConnectionPoints('register') },
      { id: 'mem1', type: 'memory', x: 500, y: 200, width: 100, height: 60, icon: '💾', label: 'MEMORY', connectionPoints: this.getConnectionPoints('memory') },
      { id: 'ctrl1', type: 'control-unit', x: 300, y: 100, width: 100, height: 60, icon: '🎛️', label: 'CONTROL UNIT', connectionPoints: this.getConnectionPoints('control-unit') },
      { id: 'pc1', type: 'program-counter', x: 150, y: 50, width: 100, height: 60, icon: '📍', label: 'PROGRAM COUNTER', connectionPoints: this.getConnectionPoints('program-counter') },
      { id: 'clk1', type: 'clock', x: 50, y: 100, width: 100, height: 60, icon: '⏰', label: 'CLOCK', connectionPoints: this.getConnectionPoints('clock') }
    ];

    const connections = [
      { id: 'conn1', start: { component: components[1], point: components[1].connectionPoints[1] }, end: { component: components[0], point: components[0].connectionPoints[0] } },
      { id: 'conn2', start: { component: components[2], point: components[2].connectionPoints[1] }, end: { component: components[0], point: components[0].connectionPoints[1] } },
      { id: 'conn3', start: { component: components[0], point: components[0].connectionPoints[2] }, end: { component: components[3], point: components[3].connectionPoints[1] } },
      { id: 'conn4', start: { component: components[4], point: components[4].connectionPoints[1] }, end: { component: components[0], point: components[0].connectionPoints[0] } },
      { id: 'conn5', start: { component: components[6], point: components[6].connectionPoints[0] }, end: { component: components[5], point: components[5].connectionPoints[0] } }
    ];
  ...
  createRISCProcessor() {
    const components = [
      { id: 'alu1', type: 'alu', x: 400, y: 300, width: 100, height: 60, icon: '🧮', label: 'ALU', connectionPoints: this.getConnectionPoints('alu') },
      { id: 'reg1', type: 'register', x: 200, y: 200, width: 100, height: 60, icon: '📝', label: 'REGISTER', connectionPoints: this.getConnectionPoints('register') },
      { id: 'reg2', type: 'register', x: 200, y: 300, width: 100, height: 60, icon: '📝', label: 'REGISTER', connectionPoints: this.getConnectionPoints('register') },
      { id: 'reg3', type: 'register', x: 200, y: 400, width: 100, height: 60, icon: '📝', label: 'REGISTER', connectionPoints: this.getConnectionPoints('register') },
      { id: 'mem1', type: 'memory', x: 600, y: 300, width: 100, height: 60, icon: '💾', label: 'MEMORY', connectionPoints: this.getConnectionPoints('memory') },
      { id: 'ctrl1', type: 'control-unit', x: 400, y: 100, width: 100, height: 60, icon: '🎛️', label: 'CONTROL UNIT', connectionPoints: this.getConnectionPoints('control-unit') },
      { id: 'pc1', type: 'program-counter', x: 200, y: 50, width: 100, height: 60, icon: '📍', label: 'PROGRAM COUNTER', connectionPoints: this.getConnectionPoints('program-counter') },
      { id: 'ir1', type: 'instruction-register', x: 400, y: 50, width: 100, height: 60, icon: '📋', label: 'INSTRUCTION REGISTER', connectionPoints: this.getConnectionPoints('instruction-register') },
      { id: 'mux1', type: 'multiplexer', x: 300, y: 250, width: 100, height: 60, icon: '🔀', label: 'MULTIPLEXER', connectionPoints: this.getConnectionPoints('multiplexer') },
      { id: 'adder1', type: 'adder', x: 500, y: 150, width: 100, height: 60, icon: '➕', label: 'ADDER', connectionPoints: this.getConnectionPoints('adder') }
    ];

    const connections = [
      { id: 'conn1', start: { component: components[1], point: components[1].connectionPoints[1] }, end: { component: components[8], point: components[8].connectionPoints[0] } },
      { id: 'conn2', start: { component: components[2], point: components[2].connectionPoints[1] }, end: { component: components[8], point: components[8].connectionPoints[1] } },
      { id: 'conn3', start: { component: components[8], point: components[8].connectionPoints[4] }, end: { component: components[0], point: components[0].connectionPoints[0] } },
      { id: 'conn4', start: { component: components[0], point: components[0].connectionPoints[2] }, end: { component: components[4], point: components[4].connectionPoints[1] } }
    ];
  ...
  createPipelinedCPU() {
    const components = [
      // Fetch Stage
      { id: 'pc1', type: 'program-counter', x: 100, y: 100, width: 100, height: 60, icon: '📍', label: 'PROGRAM COUNTER', connectionPoints: this.getConnectionPoints('program-counter') },
      { id: 'imem1', type: 'memory', x: 250, y: 100, width: 100, height: 60, icon: '💾', label: 'INSTRUCTION MEMORY', connectionPoints: this.getConnectionPoints('memory') },
      
      // Decode Stage
      { id: 'ir1', type: 'instruction-register', x: 100, y: 200, width: 100, height: 60, icon: '📋', label: 'INSTRUCTION REGISTER', connectionPoints: this.getConnectionPoints('instruction-register') },
      { id: 'dec1', type: 'decoder', x: 250, y: 200, width: 100, height: 60, icon: '🔍', label: 'DECODER', connectionPoints: this.getConnectionPoints('decoder') },
      { id: 'regfile', type: 'register', x: 400, y: 200, width: 100, height: 60, icon: '📝', label: 'REGISTER FILE', connectionPoints: this.getConnectionPoints('register') },
      
      // Execute Stage
      { id: 'alu1', type: 'alu', x: 100, y: 300, width: 100, height: 60, icon: '🧮', label: 'ALU', connectionPoints: this.getConnectionPoints('alu') },
      { id: 'comp1', type: 'comparator', x: 250, y: 300, width: 100, height: 60, icon: '⚖️', label: 'COMPARATOR', connectionPoints: this.getConnectionPoints('comparator') },
      
      // Memory Stage
      { id: 'dmem1', type: 'memory', x: 100, y: 400, width: 100, height: 60, icon: '💾', label: 'DATA MEMORY', connectionPoints: this.getConnectionPoints('memory') },
      { id: 'cache1', type: 'cache', x: 250, y: 400, width: 100, height: 60, icon: '⚡', label: 'CACHE', connectionPoints: this.getConnectionPoints('cache') },
      
      // Writeback Stage
      { id: 'mux1', type: 'multiplexer', x: 400, y: 400, width: 100, height: 60, icon: '🔀', label: 'MULTIPLEXER', connectionPoints: this.getConnectionPoints('multiplexer') },
      
      // Control
      { id: 'ctrl1', type: 'control-unit', x: 550, y: 250, width: 100, height: 60, icon: '🎛️', label: 'CONTROL UNIT', connectionPoints: this.getConnectionPoints('control-unit') },
      { id: 'clk1', type: 'clock', x: 550, y: 100, width: 100, height: 60, icon: '⏰', label: 'CLOCK', connectionPoints: this.getConnectionPoints('clock') }
    ];

    const connections = [
      { id: 'conn1', start: { component: components[0], point: components[0].connectionPoints[3] }, end: { component: components[1], point: components[1].connectionPoints[0] } },
      { id: 'conn2', start: { component: components[1], point: components[1].connectionPoints[2] }, end: { component: components[2], point: components[2].connectionPoints[0] } },
      { id: 'conn3', start: { component: components[2], point: components[2].connectionPoints[2] }, end: { component: components[3], point: components[3].connectionPoints[0] } },
      { id: 'conn4', start: { component: components[4], point: components[4].connectionPoints[1] }, end: { component: components[5], point: components[5].connectionPoints[0] } }
    ];
  ...
  createMicrocontroller() {
    const components = [
      // Core
      { id: 'cpu1', type: 'alu', x: 300, y: 200, width: 100, height: 60, icon: '🧮', label: 'CPU CORE', connectionPoints: this.getConnectionPoints('alu') },
      { id: 'mem1', type: 'memory', x: 450, y: 200, width: 100, height: 60, icon: '💾', label: 'FLASH MEMORY', connectionPoints: this.getConnectionPoints('memory') },
      { id: 'ram1', type: 'memory', x: 450, y: 300, width: 100, height: 60, icon: '💾', label: 'RAM', connectionPoints: this.getConnectionPoints('memory') },
      
      // I/O
      { id: 'in1', type: 'input-port', x: 150, y: 150, width: 100, height: 60, icon: '📥', label: 'INPUT PORT A', connectionPoints: this.getConnectionPoints('input-port') },
      { id: 'in2', type: 'input-port', x: 150, y: 250, width: 100, height: 60, icon: '📥', label: 'INPUT PORT B', connectionPoints: this.getConnectionPoints('input-port') },
      { id: 'out1', type: 'output-port', x: 600, y: 150, width: 100, height: 60, icon: '📤', label: 'OUTPUT PORT A', connectionPoints: this.getConnectionPoints('output-port') },
      { id: 'out2', type: 'output-port', x: 600, y: 250, width: 100, height: 60, icon: '📤', label: 'OUTPUT PORT B', connectionPoints: this.getConnectionPoints('output-port') },
      
      // Control & Timing
      { id: 'ctrl1', type: 'control-unit', x: 300, y: 100, width: 100, height: 60, icon: '🎛️', label: 'CONTROL UNIT', connectionPoints: this.getConnectionPoints('control-unit') },
      { id: 'clk1', type: 'clock', x: 150, y: 50, width: 100, height: 60, icon: '⏰', label: 'OSCILLATOR', connectionPoints: this.getConnectionPoints('clock') },
      { id: 'int1', type: 'interrupt', x: 300, y: 350, width: 100, height: 60, icon: '⚠️', label: 'INTERRUPT CONTROLLER', connectionPoints: this.getConnectionPoints('interrupt') },
      
      // Bus
      { id: 'bus1', type: 'bus', x: 300, y: 300, width: 100, height: 60, icon: '🚌', label: 'SYSTEM BUS', connectionPoints: this.getConnectionPoints('bus') }
    ];

    const connections = [
      { id: 'conn1', start: { component: components[0], point: components[0].connectionPoints[2] }, end: { component: components[10], point: components[10].connectionPoints[0] } },
      { id: 'conn2', start: { component: components[10], point: components[10].connectionPoints[1] }, end: { component: components[1], point: components[1].connectionPoints[0] } },
      { id: 'conn3', start: { component: components[10], point: components[10].connectionPoints[2] }, end: { component: components[2], point: components[2].connectionPoints[0] } },
      { id: 'conn4', start: { component: components[3], point: components[3].connectionPoints[1] }, end: { component: components[0], point: components[0].connectionPoints[0] } },
      { id: 'conn5', start: { component: components[8], point: components[8].connectionPoints[0] }, end: { component: components[7], point: components[7].connectionPoints[0] } }
    ];
